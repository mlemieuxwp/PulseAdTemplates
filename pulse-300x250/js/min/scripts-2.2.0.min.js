...
var PulseSlides=function(){function e(e,i){for(var a,r=/<%([^%>]+)?%>/g,t=/(^( )?(if|for|else|switch|case|break|{|}))(.*)?/g,l="var r=[];\n",s=0,c=function(e,i){return l+=i?e.match(t)?e+"\n":"r.push("+e+");\n":""!=e?'r.push("'+e.replace(/"/g,'\\"')+'");\n':"",c};a=r.exec(e);)c(e.slice(s,a.index))(a[1],!0),s=a.index+a[0].length;return c(e.substr(s,e.length-s)),l+='return r.join("");',new Function(l.replace(/[\r\t\n]/g,"")).apply(i)}function a(e){for(var i=e.length-1;i>0;i--){var a=Math.floor(Math.random()*(i+1)),r=e[i];e[i]=e[a],e[a]=r}return e}function r(){function r(e){e.onclick=function(e){if(o){var i=document.createElement("img");i.alt="",i.border=0,i.src=o,i.style.width="1px",i.style.height="1px",i.style.display="none",p.appendChild(i)}}}if(document.getElementById("articles")){var l,s=document.getElementById("articles"),c=s.getAttribute("data-articles"),n=s.getAttribute("data-shuffle"),d=s.getAttribute("data-urlparam"),o=s.getAttribute("data-clicktrack"),p=document.getElementsByClassName("pulse-tracking-wrapper")[0];if(c=JSON.parse(c)){n?l=a(c).slice(0,3):(l=c.slice(0,3),l.reverse());var u=e(t.articles,{articles:l,linkTarget:function(e){return this.articles[e].sponsor?"_blank":"_top"},showArticles:!0,url_param:d?"?spon_con="+d:""})}var h=s.getAttribute("data-ad");if(h=JSON.parse(h),h&&"video"===h.type&&(h.videoTracking={},h.videoTracking.trackStart=h.trackstart||null,h.videoTracking.track25=h.track25||null,h.videoTracking.track50=h.track50||null,h.videoTracking.track75=h.track75||null,h.videoTracking.track100=h.track100||null,h.videoTracking=JSON.stringify(h.videoTracking)),h){var v=e(t[h.type],{ad:h});u=v+u}var m=document.createElement("div");for(m.innerHTML=u,i=0;i<m.childNodes.length;)el=m.childNodes[i],m.removeChild(el),s.insertBefore(el,s.firstChild);if(s.querySelectorAll){var f=s.querySelectorAll("a");for(i=0;i<f.length;i++)r(f[i])}}}var t={articles:'<%if(this.showArticles) {%><%for(var index in this.articles) {%><div class="js-pulse-mobile-article <%if(this.articles[index].sponsor) {%>slick-sponsor<%}%>"><div class="pulse-mobile-desc-wrapper"><div class="pulse-mobile-desc"><a href="<%this.articles[index].url%><%this.url_param%>" class="pulse-mobile-desc-link" target="<% this.linkTarget(index) %>"><p class="pulse-mobile-desc-text"><%this.articles[index].title%></p><p class="pulse-mobile-article-readmore">READ MORE <i class="fa fa-angle-double-right fa-1"></i></p></a></div><span class="pulse-mobile-logo"></span></div><img class="pulse-mobile-slide-img" data-lazy="https://img.washingtonpost.com/wp-apps/imrs.php?src=<%this.articles[index].src%>&w=300"></div><%}%><%} else {%><p>none</p><%}%>',html:'<div data-animate="true"><%this.ad.html%></div>',iframe:'<div data-animate="true"><iframe src="<%this.ad.src%>" border="0" frameBorder="0" height="250" scrolling="no" width="300" style="border:0"></iframe></div>',image:'<div data-animate="true"><%if(this.ad.url) {%><a href="<%this.ad.url%>" target="_blank"><%}%><%if(this.ad.src) {%><img src="<%this.ad.src%>" alt="" border="0" style="border:0" /><%}%><%if(this.ad.url) {%></a><%}%></div>',video:'<div data-animate="true"><div class="pulse-player-wrapper" data-videotracking=\'<% this.ad.videoTracking %>\'><i class="fa fa-4x fa-play-circle-o pulse-player-play"></i><i class="fa fa-volume-up pulse-player-volume"></i><video id="pulse-player" class="pulse-player" data-current-time="0.25" poster="<% this.ad.poster_url %>"><%if(this.ad.src) {%><source src="<%this.ad.src%>" type="video/mp4" /><%}%></video></div></div>'};return{load:r}}();PulseSlides.load();
