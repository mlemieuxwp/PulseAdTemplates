...
var PulseSlides=function(){function e(e,i){for(var t,r=/<%([^%>]+)?%>/g,a=/(^( )?(if|for|else|switch|case|break|{|}))(.*)?/g,s="var r=[];\n",l=0,c=function(e,i){return s+=i?e.match(a)?e+"\n":"r.push("+e+");\n":""!=e?'r.push("'+e.replace(/"/g,'\\"')+'");\n':"",c};t=r.exec(e);)c(e.slice(l,t.index))(t[1],!0),l=t.index+t[0].length;return c(e.substr(l,e.length-l)),s+='return r.join("");',new Function(s.replace(/[\r\t\n]/g,"")).apply(i)}function t(e){for(var i=e.length-1;i>0;i--){var t=Math.floor(Math.random()*(i+1)),r=e[i];e[i]=e[t],e[t]=r}return e}function r(){function r(e){e.onclick=function(e){if(o){var i=document.createElement("img");i.alt="",i.border=0,i.src=o,i.style.width="1px",i.style.height="1px",i.style.display="none",p.appendChild(i)}}}if(document.getElementById("articles")){var s,l=document.getElementById("articles"),c=l.getAttribute("data-articles"),d=l.getAttribute("data-shuffle"),n=l.getAttribute("data-urlparam"),o=l.getAttribute("data-clicktrack"),p=document.getElementsByClassName("pulse-mobile-tracking")[0];if(c=JSON.parse(c)){d?s=t(c).slice(0,3):(s=c.slice(0,3),s.reverse());var u=e(a.articles,{articles:s,showArticles:!0,url_param:n?"?spon_con="+n:""})}var h=l.getAttribute("data-ad");if(h=JSON.parse(h)){var m=e(a[h.type],{ad:h});u=m+u}var f=document.createElement("div");for(f.innerHTML=u,i=0;i<f.childNodes.length;)el=f.childNodes[i],f.removeChild(el),l.insertBefore(el,l.firstChild);if(l.querySelectorAll){var v=l.querySelectorAll("a");for(i=0;i<v.length;i++)r(v[i])}}}var a={articles:'<%if(this.showArticles) {%><%for(var index in this.articles) {%><div class="js-pulse-mobile-article <%if(this.articles[index].sponsor && this.articles[index].content_from) {%>slick-sponsor<%}%>"><div class="pulse-mobile-desc-wrapper"><div class="pulse-mobile-desc"><a href="<%this.articles[index].url%><%this.url_param%>" class="pulse-mobile-desc-link" target="_top"><%if(this.articles[index].sponsor && this.articles[index].content_from) {%><label class="pulse-mobile-desc-label">content from <%this.articles[index].content_from%></label><%}%><p class="pulse-mobile-desc-text"><%this.articles[index].title%></p><p class="pulse-mobile-article-readmore">READ MORE <i class="fa fa-angle-double-right fa-1"></i></p></a></div></div><%if(this.articles[index].src && !this.articles[index].sponsor) {%><img class="pulse-mobile-slide-img" data-lazy="https://img.washingtonpost.com/wp-apps/imrs.php?src=<%this.articles[index].src%>&w=300"><%}%></div><%}%><%} else {%><p>none</p><%}%>',html:'<div data-animate="true"><%this.ad.html%></div>',iframe:'<div data-animate="true"><iframe src="<%this.ad.src%>" border="0" frameBorder="0" height="250" scrolling="no" width="300" style="border:0"></iframe></div>',image:'<div data-animate="true"><%if(this.ad.url) {%><a href="<%this.ad.url%>"><%}%><%if(this.ad.src) {%><img src="<%this.ad.src%>" alt="" border="0" style="border:0" /><%}%><%if(this.ad.url) {%></a><%}%></div>',video:'<div data-animate="true"><div class="pulse-player-wrapper"><i class="fa fa-4x fa-play-circle-o pulse-player-play"></i><i class="fa fa-volume-up pulse-player-volume"></i><video id="pulse-player" class="pulse-player" data-current-time="0.25"><source src="<%this.ad.src%>" type="video/mp4" /></video></div></div>'};return{load:r}}();PulseSlides.load();
...
var PulseTracking=function(){function e(e){var t=new Image;t.alt="",t.border=0,t.height=1,t.width=1,t.style.display="none",e&&(e=e.replace(/%%CACHEBUSTER%%/gi,""),e+=Math.floor(1e12*Math.random())+"?",t.src=e,l.appendChild(t))}function t(e){var t=(e/4).toFixed(1);sessionStorage.setItem("one",t),sessionStorage.setItem("two",(2*t).toFixed(1)),sessionStorage.setItem("three",(3*t).toFixed(1))}function n(){var t=a.currentTime.toFixed(1);c&&t>=parseInt(sessionStorage.getItem("one"))?(e(c),sessionStorage.setItem("one",null)):g&&t>=parseInt(sessionStorage.getItem("two"))?(e(g),sessionStorage.setItem("two",null)):k&&t>=parseInt(sessionStorage.getItem("three"))&&(e(k),sessionStorage.setItem("three",null))}function i(){e(u)}function s(){e(d),t(this.duration)}function o(){a&&(a.addEventListener("ended",i,!1),a.addEventListener("timeupdate",n,!1),a.addEventListener("play",s,!1))}function r(){o()}var a=document.getElementById("pulse-player"),l=document.getElementsByClassName("pulse-tracking-wrapper")[0];if("undefined"!=typeof PulseVideoTracking)var d=PulseVideoTracking&&PulseVideoTracking.trackStart||null,u=PulseVideoTracking&&PulseVideoTracking.track100||null,c=PulseVideoTracking&&PulseVideoTracking.track25||null,g=PulseVideoTracking&&PulseVideoTracking.track50||null,k=PulseVideoTracking&&PulseVideoTracking.track75||null;return{init:r}}();document.addEventListener("DOMContentLoaded",PulseTracking.init(),!1);