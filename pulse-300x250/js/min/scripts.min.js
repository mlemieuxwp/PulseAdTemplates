...
var PulseArticles=function(){function e(e,i){for(var t,r=/<%([^%>]+)?%>/g,s=/(^( )?(if|for|else|switch|case|break|{|}))(.*)?/g,l="var r=[];\n",a=0,n=function(e,i){return l+=i?e.match(s)?e+"\n":"r.push("+e+");\n":""!=e?'r.push("'+e.replace(/"/g,'\\"')+'");\n':"",n};t=r.exec(e);)n(e.slice(a,t.index))(t[1],!0),a=t.index+t[0].length;return n(e.substr(a,e.length-a)),l+='return r.join("");',new Function(l.replace(/[\r\t\n]/g,"")).apply(i)}function t(e){for(var i=e.length-1;i>0;i--){var t=Math.floor(Math.random()*(i+1)),r=e[i];e[i]=e[t],e[t]=r}return e}function r(){if(document.getElementById("articles")){var r,l,a=document.getElementById("articles"),n=a.getAttribute("data-articles"),c=a.getAttribute("data-shuffle");for(n=JSON.parse(n),c?(l=t(n),r=l.slice(1,4)):r=n;a.firstChild;)a.removeChild(a.firstChild);var o=e(s,{articles:r,showArticles:!0}),d=document.createElement("div");for(d.innerHTML=o,i=0;i<d.childNodes.length;)el=d.childNodes[i],d.removeChild(el),a.parentNode.insertBefore(el,a);var p=a.parentNode;p.removeChild(a)}}var s='<%if(this.showArticles) {%><%for(var index in this.articles) {%><div><div class="pulse-mobile-desc-wrapper"><div class="pulse-mobile-desc"><a href="<%this.articles[index].url%>" class="pulse-mobile-desc-link" target="_top"><p class="pulse-mobile-desc-text"><%this.articles[index].title%></p><p class="pulse-mobile-article-readmore">READ MORE <i class="fa fa-angle-double-right fa-1"></i></p></a></div></div><img class="pulse-mobile-slide-img" data-lazy="https://img.washingtonpost.com/wp-apps/imrs.php?src=<%this.articles[index].src%>&w=300"></div><%}%><%} else {%><p>none</p><%}%>';return{loadArticles:r}}();PulseArticles.loadArticles();
